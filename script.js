...
let goalExperienceDismissed = false;
...
async function runGoalCountdown({ preview = false, preserve = false } = {}) {
  ...
  setConnectionStatus(
    ...
    preview ? "Preparando a prévia de 1 milhão..." : "Meta alcançada! Preparando comemoração..."
  );
...
function dismissGoalExperience() {
  if (goalCountdownRunning || !document.body.classList.contains("goal-achieved")) return;

  goalExperienceDismissed = true;
  stopCelebrationParticles();
  setGoalVisualState(false, { preview: false, entering: false });
  ...
  setConnectionStatus(
    "is-connected",
    liveRevenueValue >= GOAL ? "Meta de R$ 1 milhão atingida!" : `Planilha atualizada às ${time}`
  );
}
...
function restoreRealDashboard() {
  ...
  if (liveRevenueValue >= GOAL) {
    ...
    setConnectionStatus("is-connected", "Meta de R$ 1 milhão atingida!");
    return;
  }
...
function activateGoalExperience({ preview = false, preserve = false } = {}) {
  ...
  hasCelebratedMillion = true;
  if (!preserve) {
    setConnectionStatus(preview ? "is-demo" : "is-connected", preview ? "Prévia da animação de 1 milhão" : "Meta de R$ 1 milhão atingida!");
  }
}
...
function maybeCelebrateGoal(revenue) {
  ...
  if (revenue >= GOAL) {
    if (goalExperienceDismissed) return;
...
async function updateRevenue({ initial = false } = {}) {
  ...
  try {
    ...
    if (!previewActive && !goalCountdownRunning) {
      const firstGoalReach = liveRevenueValue >= GOAL && !hasCelebratedMillion && !goalExperienceDismissed;
      const suppressGoalState = goalExperienceDismissed && liveRevenueValue >= GOAL;
      renderDashboard(liveRevenueValue, {
        animate: true,
        honorPreview: firstGoalReach || suppressGoalState
      });
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  document.addEventListener("click", event => {
    if (goalCountdownRunning) return;
    if (!document.body.classList.contains("goal-achieved")) return;
    dismissGoalExperience();
  });

  document.addEventListener("keydown", event => {
    if (event.key === "Escape") dismissGoalExperience();
  });
